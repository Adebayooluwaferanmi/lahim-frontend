import React, { useState } from 'react'
import { useTranslation } from 'react-i18next'
import { Button, Table, Alert, Spinner, Panel, Row, Column } from '@hospitalrun/components'
import {
  useLocations,
  useCreateLocation,
  useDeleteLocation,
  Location,
} from '../hooks/useSettings'
import TextInputWithLabelFormGroup from '../components/input/TextInputWithLabelFormGroup'

const Locations = () => {
  const { t } = useTranslation()
  const { data, isLoading } = useLocations()
  const { mutate: createLocation } = useCreateLocation()
  const { mutate: deleteLocation } = useDeleteLocation()

  const [showForm, setShowForm] = useState(false)
  const [formData, setFormData] = useState<Partial<Location>>({ name: '', description: '', address: '' })

  const handleCreate = () => {
    createLocation(formData, {
      onSuccess: () => {
        setShowForm(false)
        setFormData({ name: '', description: '', address: '' })
      },
    })
  }

  const handleDelete = (id: string) => {
    if (window.confirm(t('settings.confirmDelete', 'Are you sure you want to delete this item?'))) {
      deleteLocation(
        {},
        {
          onSuccess: () => {},
        }
      )
    }
  }

  if (isLoading) {
    return <Spinner color="blue" loading size={[10, 25]} type="ScaleLoader" />
  }

  const locations = data?.locations || []

  return (
    <div>
      <div className="mb-3">
        <Button color="success" icon="add" onClick={() => setShowForm(!showForm)}>
          {String(t('settings.addLocation', 'Add Location'))}
        </Button>
      </div>

      {showForm && (
        <Panel title={String(t('settings.newLocation', 'New Location'))}>
          <Row>
            <Column md={6}>
              <TextInputWithLabelFormGroup
                label={String(t('settings.name', 'Name'))}
                name="name"
                value={formData.name || ''}
                onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                isRequired
                isEditable
              />
            </Column>
            <Column md={6}>
              <TextInputWithLabelFormGroup
                label={String(t('settings.address', 'Address'))}
                name="address"
                value={formData.address || ''}
                onChange={(e) => setFormData({ ...formData, address: e.target.value })}
                isEditable
              />
            </Column>
          </Row>
          <Row>
            <Column>
              <TextInputWithLabelFormGroup
                label={String(t('settings.description', 'Description'))}
                name="description"
                value={formData.description || ''}
                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                isEditable
              />
            </Column>
          </Row>
          <Row>
            <Column>
              <Button color="success" onClick={handleCreate}>
                {String(t('actions.save', 'Save'))}
              </Button>
              <Button outlined color="secondary" onClick={() => setShowForm(false)} className="ml-2">
                {String(t('actions.cancel', 'Cancel'))}
              </Button>
            </Column>
          </Row>
        </Panel>
      )}

      <Table
        data={locations.map((loc) => ({
          id: loc.id || loc._id,
          name: loc.name,
          address: loc.address || '-',
          description: loc.description || '-',
          actions: (
            <Button color="danger" onClick={() => handleDelete(loc.id || loc._id || '')}>
              {String(t('actions.delete', 'Delete'))}
            </Button>
          ),
        }))}
        columns={[
          { label: t('settings.name', 'Name'), key: 'name' },
          { label: t('settings.address', 'Address'), key: 'address' },
          { label: t('settings.description', 'Description'), key: 'description' },
          { label: t('actions.label', 'Actions'), key: 'actions' },
        ]}
      />
    </div>
  )
}

export default Locations

